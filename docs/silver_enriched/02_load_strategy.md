...
Zeile im Ziel erzeugen** — z. B. ein Kapitel ohne überprüfbare Behauptungen bei `fact_checker`.
...
**dem Zeitpunkt, an dem er selbst zuletzt erfolgreich gelaufen ist** — unabhängig davon, ob dabei
Zeilen geschrieben wurden oder nicht.
...
- Es gibt **keine Aggregation mehr** (`MAX(...)` über das Ziel, `HAVING`, `LEFT JOIN`). Die Abfrage
  ist ein einfacher Join + `WHERE`, unabhängig davon ob ein Step 0, 1 oder N Zeilen pro
  Quell-Datensatz schreibt.
- Gilt **identisch** für alle vier Steps und alle drei Embedding-Ebenen — keine Sonderfälle mehr.
...
- Wird zwischen Start und Ende eines Laufs ein Datensatz neu vorverarbeitet
  ...
  diesen Datensatz **fälschlich als "schon erledigt" markieren** — er wurde aber gar nicht gelesen.
- Mit `start_ts` als Watermark wird ein solcher Datensatz beim **nächsten** Lauf einfach erneut
  ...
  (idempotent durch `ON CONFLICT ... DO UPDATE`) — silent data loss ist damit ausgeschlossen.
...
- Früher: `datetime.now(timezone.utc)` in Python — Risiko von **Uhren-Drift** zwischen App-Server
  und Datenbank, wenn dieser Wert später in SQL mit DB-eigenen Zeitstempeln verglichen wird.
...
- Wiederholte Läufe sind idempotent und günstig: Kapitel/Zeilen, die bereits seit dem letzten
  erfolgreichen Lauf nicht mehr verändert wurden, werden **nicht** erneut durch teure
  LLM-/Embedding-/Audio-Aufrufe geschickt — **auch wenn sie keine Zielzeile erzeugt haben**
...
- Jeder Step committet **alle** seine Ergebnisse in einem einzigen `executemany` + einem finalen
  `conn.commit()` (kein partielles Fortschreiben). Schlägt ein Lauf fehl, gibt es daher ohnehin
...
- Wird `--batch-id` explizit übergeben, übernimmt der Step diesen Batch nur zum Markieren der
  geschriebenen Zeilen. Er legt dann **keinen eigenen** Batch-Eintrag an und rückt die Watermark
  dadurch **nicht** vor — das obliegt dem Aufrufer.
- `--dry-run` legt ebenfalls keinen Batch-Eintrag an → rückt die Watermark nicht vor.
...
- `start_ts` statt `fin_ts`, um keine während des Laufs aktualisierten Datensätze zu verschlucken.
- `processing_updated_at` kommt aus `SELECT NOW()` (DB-Uhr), einmal pro Lauf, geteilt über alle
  Steps und Zeilen dieses Laufs.
- Nur `status='success'`-Batches zählen als Watermark; fehlgeschlagene/Dry-Run/`--batch-id`-Läufe
  rücken sie nicht vor.
