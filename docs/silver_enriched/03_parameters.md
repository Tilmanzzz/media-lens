...
| `backend`                                             | `remote` (Code-Default: `local`)      | `local` (Wav2Vec2-Modell im selben Prozess) oder `remote` (HTTP-Endpunkt, z. B. Spark-Device). Bei `remote` werden `model_id`/`cache_dir` ignoriert und `torch`/`transformers` werden nicht geladen          |
| `remote_endpoint_url`                                 | `http://xxx:8000/predict`             | Nur bei `backend: remote`: Ziel-URL, die den Audio-Snippet per `multipart/form-data` (`file=@...`) entgegennimmt und ein JSON-Objekt `{"predictions": [{"label": ..., "emotion": ..., "score": ...}, ...], "top": {"label": ..., "emotion": ..., "score": ...}}` zurückgibt. Es wird `top` ausgewertet (Fallback: höchster `score` in `predictions`) |
| `remote_timeout`                                      | `30.0`                                | Nur bei `backend: remote`: Timeout (Sekunden) für den HTTP-Request                                                                                                                                           |
| `logging_enabled`, `log_level`, `log_dir`, `log_file` | -                                     | Logging                                                                                                                                                                                                      |
