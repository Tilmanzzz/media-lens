...
def make_episode(pipeline_run_id: uuid.UUID, **kwargs) -> NewEpisode:
...
class TestGet:
    def test_returns_episode_for_valid_id(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        result = get(episode_id, conn)
        assert result is not None
        assert result.id == episode_id

    def test_returns_none_for_nonexistent_id(self, conn, pipeline_run_id: uuid.UUID):
    ...
    def test_returns_correct_fields(self, conn, pipeline_run_id: uuid.UUID):
        ...
        result = get(episode_id, conn)
        assert result.title == "My Podcast"
        assert result.podcast_id == test_podcast_id
        assert result.audio_path == "bronze/ep1.mp3"

    def test_defaults_status_to_pending(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        result = get(episode_id, conn)
        assert result.status == EpisodeStatus.PENDING

    def test_generated_fields_are_set(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        result = get(episode_id, conn)
...
class TestSave:
    def test_returns_uuid_id(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        assert isinstance(episode_id, uuid.UUID)

    def test_minimal_episode_only_title(self, conn, pipeline_run_id: uuid.UUID):
    ...
    def test_full_episode_all_fields(self, conn, pipeline_run_id: uuid.UUID):
    ...
    def test_two_saves_return_different_ids(self, conn, pipeline_run_id: uuid.UUID):
        id1 = save(make_episode(pipeline_run_id), conn)
        id2 = save(make_episode(pipeline_run_id), conn)
        assert id1 != id2

    def test_requires_title(self, conn, pipeline_run_id: uuid.UUID):
...
class TestUpdate:
    def test_update_single_field(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        update(episode_id, {"audio_path": "bronze/updated.mp3"}, conn)
        result = get(episode_id, conn)
        assert result.audio_path == "bronze/updated.mp3"

    def test_update_multiple_fields(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        update(
            episode_id,
            {
    ...
    def test_update_status(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        update(episode_id, {"status": EpisodeStatus.DONE.value}, conn)
        result = get(episode_id, conn)
        assert result.status == EpisodeStatus.DONE

    def test_update_empty_dict_is_noop(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id, title="Original"), conn)
        update(episode_id, {}, conn)
        result = get(episode_id, conn)
        assert result.title == "Original"

    def test_update_invalid_field_raises(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        with pytest.raises(ValueError, match="Invalid update fields"):
            update(episode_id, {"nonexistent_field": "value"}, conn)

    def test_update_id_field_raises(self, conn, pipeline_run_id: uuid.UUID):
        episode_id = save(make_episode(pipeline_run_id), conn)
        with pytest.raises(ValueError):
            update(episode_id, {"id": uuid.uuid4()}, conn)
...
class TestGetList:
    def test_returns_list(self, conn, pipeline_run_id: uuid.UUID):
        result = get_list(conn=conn)
        assert isinstance(result, list)

    def test_returns_saved_episodes(self, conn, pipeline_run_id: uuid.UUID):
    ...
    def test_ordered_by_ingested_at_desc(self, conn, pipeline_run_id: uuid.UUID):
        for i in range(3):
            save(make_episode(pipeline_run_id, title=f"Ep {i}"), conn)
        results = get_list(batch_id=pipeline_run_id, conn=conn)
        # Verify ordering logic if necessary
