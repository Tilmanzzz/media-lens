...
from silver_enriched.emotion_analyser.emotion_analyser import (
    EmotionAnalyser,
    RemoteEmotionEndpointUnavailable,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run emotion scoring against transcript lines (full or delta)."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--log-file",
        default="emotion_scoring_pipeline.log",
        help="Pipeline log file name",
    )
    parser.add_argument(
        "--cache-dir",
        default=None,
        help="Optional persistent cache directory for episode audio files",
    )
...
def _extract_line_segment(
    ...
    if duration <= 0:
        raise ValueError(
            f"Invalid transcript line duration: start={start_time} end={end_time}"
        )
...
def run_step(conn, ctx: LoadContext, args: argparse.Namespace) -> None:
    ...
    with pipeline_batch_scope(
        ...
        cache_dir_arg = (
            ...
            or os.environ.get("EMOTION_CACHE_DIR")
            or analyser.config.audio_cache_dir
        )
        ...
        with tempfile.TemporaryDirectory(prefix="emotion_scoring_") as temp_dir:
            ...
            for episode_id, episode_chunks in episodes.items():
                ...
                if not audio_key:
                    logger.warning(
                        "emotion_scoring: skipping episode, no audio_key episode_id=%s",
                        episode_id,
                    )
                    continue
                ...
                if source_audio is None:
                    filename = (
                        audio_key.strip("/\\").replace("/", "__").replace("\\", "__")
                    )
                    try:
                        if cache_root is not None:
                            ...
                            if not cached_path.exists():
                                ...
                                download_object_to_path(
                                    minio_client, audio_key, cached_path, logger=logger
                                )
                            else:
                                logger.info(
                            ...
                            download_object_to_path(
                                minio_client, audio_key, source_audio, logger=logger
                            )
