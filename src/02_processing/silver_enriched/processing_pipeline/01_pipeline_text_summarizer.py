...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--log-file",
        default="text_summarizer_pipeline.log",
...
def run_step(conn, ctx: LoadContext, args: argparse.Namespace) -> None:
    ...
    with pipeline_batch_scope(
        ...
        ep_count = update_episode_summaries(
            conn, episode_summaries, batch_id, ctx.processing_update_ts
        )
        ch_count = update_chapter_summaries(
            conn, chapter_summaries, batch_id, ctx.processing_update_ts
        )
        conn.commit()
        logger.info(
